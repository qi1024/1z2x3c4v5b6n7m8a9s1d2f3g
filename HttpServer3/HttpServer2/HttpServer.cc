...
namespace muduo{
  namespace net{
    ...
    void defaultHttpCallback(HttpRequest&, HttpResponse* resp){
...
HttpServer::HttpServer(EventLoop* loop,
  ...
  : server_(loop, listenAddr, name, option),
    httpCallback_(net::detail::defaultHttpCallback)
{
...
void HttpServer::onRequest(const TcpConnectionPtr& conn, HttpRequest& req){
  ...
  Pretreat pretreat(conn,req,response);
  pretreat.work(conn,req,response);
